...
%% ---- TRAIN spatial ICA (FastICA on SVD-reduced subspace) ----
[G,~,~] = svds(Y,K);
Ss = G'*Y;
[SSs,A,~] = fastica(Ss, 'numOfIC', K, 'approach','symm', 'g','tanh', 'verbose','off');
X = SSs;
D = Y*SSs';
D = D*diag(1./sqrt(sum(D.*D)));
Wdec = D; Zs = X;                                  % map to SAE variable names
K = size(Zs,1);                                    % sync K to # components returned
relErr = norm(Y-D*X,'fro')/norm(Y,'fro');
fprintf('\nICA trained. relErr=%.3f | components=%d | avg nnz/col=%.1f\n', ...
        relErr, K, mean(sum(abs(X)>1e-7,1)));
...
fprintf('Cysteine atoms found (ICA): '); fprintf('%d ',ATOMS); fprintf('\n');
...
for sN=1:nA
    ...
    title(sprintf('ICA atom %d (cysteine)   enrC = %.0fx', at, enrC(at)));
...
%% ============ Part 2b: cysteine atoms vs DISULFIDE annotation (KEY) ========
...
if isempty(dcol)
    ...
    fprintf('\n=== CYSTEINE ATOMS vs DISULFIDE STATUS (ICA, label col %d) ===\n', dcol);
    ...
    fprintf('(>1x = disulfide-biased; <1x = disulfide-depleted; a biased atom under ICA cannot be SAE absorption)\n');
end
...
fprintf('\nICA decoder cosines |cos(D_i,D_j)| among cysteine atoms (reference only):\n');
fprintf('%8s',''); for j=1:nA, fprintf('   atom%-3d',ATOMS(j)); end; fprintf('\n');
for i=1:nA
    fprintf('atom%-4d',ATOMS(i));
    for j=1:nA
...
title(sprintf('ICA: all %d atoms by dominant amino acid  [%d identity / %d diffuse]',K,nIdent,K-nIdent));
yline(nIdent+0.5,'r-','LineWidth',1.2);

fprintf('\n=== ATOM CENSUS (ICA, K=%d, enr>=%g x) ===\n',K,ENR_THR);
...
%% ============ Part 5: STRUCTURE CENSUS vs Swiss-Prot (label ICA) ===========
...
fprintf('\n=== STRUCTURE CENSUS (ICA, K=%d, enr>=%gx, >=%d hits, purity>=%.2f) ===\n', K, STR_THR, MINPOS, PUR_THR);
...
if nP>0
    ...
    for sN=1:nP
        ...
        ylim([0 1.15]); xlim([1 N]); ylabel('|act|');
        title(sprintf('ICA atom %d  ->  %s   (enr %.0fx, purity %.2f)', at, lnames{domCon(at)}, conEnr(at), conPur(at)));
        up=unique(pS,'stable'); mass=zeros(numel(up),1);
        for i=1:numel(up), mass(i)=sum(a(pS==up(i))); end
        [~,oi]=sort(mass,'descend');
        for r=1:min(NLAB,numel(up))
...
fprintf('\n=== FRACTIONATION SWEEP + CHANCE-PARTITION NULL (ICA, K=%d, layer=%d) ===\n', K, LAYER);
...
nFrac  = sum(isFrac);
fprintf('\n>>> ICA: %d of 20 amino acids fractionate: >=2 atoms, residue-disjointness BELOW chance (z<-2), purity>=%.2f\n', ...
        nFrac, PUR_MIN);
...
EXPORT_ATOMS = [22 48 150];   % disulfide, transmembrane, DNA-binding (edit to your atom indices)
